...
<p class="muted">Laporan tersedia sesuai role akun aktif.</p>
</div>
...
@forelse($reports[$key] as $row)<tr>@foreach($row as $column => $value)<td>@if(is_numeric($value) && (str_contains($column, 'jumlah') || str_contains($column, 'total') || $column === 'selisih') && !str_contains($column, 'pemakaian'))Rp {{ number_format((float) $value, 0, ',', '.') }}@elseif(is_numeric($value) && str_contains($column, 'pemakaian')){{ number_format((float) $value, 0, ',', '.') }}@else{{ $value }}@endif</td>@endforeach</tr>@empty<tr><td colspan="20">@include('layouts.partials.empty-state', ['message' => 'Tidak ada data.'])</td></tr>@endforelse
</tbody></table></div>
</div>
@endforeach
@endsection
