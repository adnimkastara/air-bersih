...
<body>
    ...
    <form method="POST" action="{{ route('meter_records.store') }}">
        @csrf
        ...
        <select name="pelanggan_id" required>
            ...
            @foreach($pelanggans as $pelanggan)
                <option value="{{ $pelanggan->id }}" {{ old('pelanggan_id') == $pelanggan->id ? 'selected' : '' }}>{{ $pelanggan->name }}</option>
            @endforeach
        </select>
        ...
        <input type="number" name="meter_previous_month" value="{{ old('meter_previous_month') }}" required>

        <label>Meter Bulan Ini</label>
        <input type="number" name="meter_current_month" value="{{ old('meter_current_month') }}" required>
