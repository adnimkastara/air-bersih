...
<div class="grid-2">
<div><label>Kode Pelanggan</label><input type="text" value="{{ $pelanggan->kode_pelanggan }}" readonly></div>
...
<input id="latitude" type="hidden" name="latitude" value="{{ old('latitude', $pelanggan->latitude) }}">
<input id="longitude" type="hidden" name="longitude" value="{{ old('longitude', $pelanggan->longitude) }}">
<div class="full"><label>Ubah Titik di Peta</label><div style="display:flex;gap:8px;margin-bottom:8px;"><button class="btn btn-outline btn-sm" type="button" id="useMyLocation">Gunakan Lokasi Saya</button><span id="coordLabel">Belum memilih titik.</span></div><div id="map" style="height:330px;border:1px solid var(--line);border-radius:12px;"></div></div>
</div>
...
const initialLat = Number(@json(old('latitude', $pelanggan->latitude ?: null))); const initialLng = Number(@json(old('longitude', $pelanggan->longitude ?: null)));
